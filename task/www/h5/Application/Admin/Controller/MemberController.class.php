...
class MemberController extends ThinkController {
    ...
    */
    public function user_info($p=0){
        ...
        if(isset($_REQUEST['nickname'])){
            $map['tab_user.nickname'] = array('like','%'.$_REQUEST['nickname'].'%');
        ...
        if(isset($_REQUEST['start']) && isset($_REQUEST['end'])){
            $map['register_time'] = array('BETWEEN',array(strtotime($_REQUEST['start']),strtotime($_REQUEST['end'])));
    ...
    */
    public function login_record($p=1){
        ...
        if(isset($_REQUEST['nickname'])){
            $map['user_nickname']=array('like','%'.$_REQUEST['nickname'].'%');
            unset($_REQUEST['nickname']);
        }
        $extend=array();
        $extend['map']=$map;
    ...
    public function edit($id=null){
    	if(IS_POST){
            ...
            if(strlen($pwd)>0&&strlen($pwd)<6||strlen($pwd)>15){
                $this->error("密码长度6-15个字符");
            }
            $res = $member->updateInfo($data);
            if($res !== false){
                if(C('UC_SET')==1&&!empty($pwd)){
