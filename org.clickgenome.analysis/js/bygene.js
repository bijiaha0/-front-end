var beesworm = new Object({
    ...
    URL: {
        ...
        CHECK_URL: function(geneName) {
            return beesworm.URL.BASE_URL() + geneName + "/" + $("#dataType").val() + "/" + $("#newOrOld").val() + "/gene/check";
        },
        BEESWORM: function(genename, cancertype) {
            ...
            if ($("#simple_nonmalignant").is(':checked') && $("#simple_tumor").is(':checked')) {
                return beesworm.URL.BASE_URL() + genename + "/" + cancertype + "/" + $("#dataType").val() + "/" + $("#newOrOld").val() + "/" + value + "/bygene";
            } else if ($("#simple_tumor").is(':checked')) {
                return beesworm.URL.BASE_URL() + genename + "/" + cancertype + "/" + $("#dataType").val() + "/" + $("#newOrOld").val() +  "/" + value + "/bygene/tumor";
            } else if ($("#simple_nonmalignant").is(':checked')) {
                return beesworm.URL.BASE_URL() + genename + "/" + cancertype + "/" + $("#dataType").val() + "/" + $("#newOrOld").val() + "/" + value + "/bygene/nonmalignant";
            } else {
                return beesworm.URL.BASE_URL() + genename + "/" + cancertype + "/" + $("#dataType").val() + "/" + $("#newOrOld").val() + "/" + value + "/bygene";
            }
        },
        TTEST_URL : function(){
            return beesworm.URL.BASE_URL() + "test";
        }
    },
    Operate: {
        BEESWORM: function(genename, cancertype) {
            ...
            }else if(beesworm.Utils.VerifyInput(genename)){
                $("#bt_draw").disabled=false;
                $("#echarts").html(
                    ...
                    $.get(beesworm.URL.CHECK_URL(beesworm.Utils.takeFirst(genename)), {}, function(data) {
                        //第一个if:对输入的检查
                        if (data.data) {
                            $("#echarts").html('<div class="spinner">' +
                                '<div class="rect1"></div>' +
                                ...
                                '</div><p class="col-md-12" style="text-align:center;color:rgb(61,132,193);">Loading... </br>Due to the big data size and the internet speed, this may take a while.</p>');
                            //ajax、get请求数据
                            $.get(beesworm.URL.BEESWORM(beesworm.Utils.takeFirst(genename), cancertype), {}, function(data, status) {
                                if (status) {
                                    ...
                                    //pointSpread填充
                                    var pointSpreadData = beesworm.Utils.pointSpread(resultData);
                                    ...
                                    beesworm.View.draw(flag, beesworm.Utils.takeFirst(genename), cancertype, resultData, beesStyle, is_reRender);
                                } else {
    ...
    Utils: {
        ...
        pointSpread:function(data){
            ...
            for(var kk = 0; kk < data.length; kk++){
                ...
                for(var bjh=0; bjh < intermediateData.length; bjh++){
                    finalData.push(beesworm.Utils.normalize(intermediateData[bjh], Math.max.apply(null,intermediateData), Math.min.apply(null,intermediateData)));
                }
        ...
        tableData:function(up,q3,avg,std,mid,q1,down,xmin){
            ...
            viewData =[
                [{
                    coord: [xmin, beesworm.Utils.getResult(up,4)],
                    symbol: 'none'
                }, {
                    coord: [xmin, beesworm.Utils.getResult(up,4)],
                    symbol: 'none'
                ...
                [{
                    coord: [xmin, beesworm.Utils.getResult(q3,4)],
                    symbol: 'none'
                }, {
                    coord: [xmin, beesworm.Utils.getResult(q3,4)],
                    symbol: 'none'
                ...
                [{
                    coord: [xmin, beesworm.Utils.getResult(mid,4)],
                    symbol: 'none'
                }, {
                    coord: [xmin, beesworm.Utils.getResult(mid,4)],
                    symbol: 'none'
                ...
                [{ //q1
                    coord: [xmin, beesworm.Utils.getResult(q1,4)],
                    symbol: 'none'
                }, {
                    coord: [xmin, beesworm.Utils.getResult(q1,4)],
                    symbol: 'none'
                ...
                [{ //down
                    coord: [xmin, beesworm.Utils.getResult(down,4)],
                    symbol: 'none'
                }, {
                    coord: [xmin, beesworm.Utils.getResult(down,4)],
                    symbol: 'none'
                ...
                [{ //avg
                    coord: [xmin, beesworm.Utils.getResult(avg,4)],
                    symbol: 'none'
                }, {
                    coord: [xmin, beesworm.Utils.getResult(avg,4)],
                    symbol: 'none'
                ...
                [{ //std
                    coord: [xmin, beesworm.Utils.getResult(std,4)],
                    symbol: 'none'
                }, {
                    coord: [xmin, beesworm.Utils.getResult(std,4)],
                    symbol: 'none'
        ...
        ttest: function(data, gene1, gene2) {
            var testData_=beesworm.Utils.getTestData(data, gene1, gene2);
            //json序列化
            //var jsonObj=json.stringify(testlist);
            var result;
            $.ajax({
                url : beesworm.URL.TTEST_URL(),
                async : false,
                type : "post",
                dataType: "json",
        ...
        getVar:function(valueList){
            var devAll=0;
            var mean=beesworm.Utils.getMean(valueList);
            for(var i=0;i<valueList.length;i++){
    ...
    View: {
        ...
        draw: function(flag , genename, cancertype, data, beesStyle, is_reRender) {
            $("#echarts").html('<div class="spinner">' +
                '<div class="rect1"></div>' +
                '<div class="rect2"></div>' +
                '<div class="rect3"></div>' +
            ...
            if(ttest_gene1 != "" & ttest_gene2 != "") {
                t_value = beesworm.Utils.ttest(data, ttest_gene1, ttest_gene2);
            }
            ...
            if (beesStyle.length == 0) {
                ...
                beesStyle[beesworm.Utils.search(glist_ex_, $("#bees_which_select").val())][0] = $("#pointColor_select").val();
                beesStyle[beesworm.Utils.search(glist_ex_, $("#bees_which_select").val())][1] = $("#pointSize_select").val();
                beesStyle[beesworm.Utils.search(glist_ex_, $("#bees_which_select").val())][2] = $("#pointStyle_select").val();
                beesStyle[beesworm.Utils.search(glist_ex_, $("#bees_which_select").val())][3] = $("#boxColor_select").val();
                beesStyle[beesworm.Utils.search(glist_ex_, $("#bees_which_select").val())][4] = $("#boxStyle_select").val();
                beesStyle[beesworm.Utils.search(glist_ex_, $("#bees_which_select").val())][5] = $("#box_lineWidth_select").val()*1;
                beesStyle[beesworm.Utils.search(glist_ex_, $("#bees_which_select").val())][6] = $("#lineColor_select").val();
                beesStyle[beesworm.Utils.search(glist_ex_, $("#bees_which_select").val())][7] = $("#lineStyle_select").val();
                beesStyle[beesworm.Utils.search(glist_ex_, $("#bees_which_select").val())][8] = $("#mean_lineWidth_select").val()*1;
            }
            /**样式数组初始化完毕*/
            /**监听重新渲染按钮 重新渲染*/
            $("#btn_Rerender").click(function() {
                ...
                //总共几个图
                var i =beesworm.Utils.search(glist_ex_, $("#bees_which_select").val());
            ...
            if(data.length !=0){
                ...
                var avg= beesworm.Utils.getMean(value_list);
                var std =beesworm.Utils.getVar(value_list);
                //cutoff统计百分比
                ...
                if($("#boxplot").is(':checked')) {
                    if($("#mean").is(':checked') || $("#mean1").is(':checked') || $("#mean2").is(':checked') || $("#mean3").is(':checked')) {
                        linedata = beesworm.Utils.box(up, q3, avg, std, mid, q1, down, xmin, xmax, cutoffall, Xmax, beesStyle[dd][3], beesStyle[dd][4], beesStyle[dd][5], beesStyle[dd][6],beesStyle[dd][7],beesStyle[dd][8]);
                        tabledata = beesworm.Utils.tableData(up, q3, avg, std, mid, q1, down, xmin);
                    }else{
                        linedata = beesworm.Utils.box(up, q3, "", std, mid, q1, down, xmin, xmax, cutoffall, Xmax, beesStyle[dd][3], beesStyle[dd][4], beesStyle[dd][5], beesStyle[dd][6], beesStyle[dd][7], beesStyle[dd][8]);
                        tabledata = beesworm.Utils.tableData(up, q3, avg, std, mid, q1, down, xmin);
                    }
                }else{
                    linedata = beesworm.Utils.box("", "", avg, std, "", "", "", xmin, xmax, cutoffall, Xmax, "", "", "", beesStyle[dd][6], beesStyle[dd][7],beesStyle[dd][8]);
                    tabledata = beesworm.Utils.tableData(up, q3, avg, std, mid, q1, down, xmin);
                }
                //构造cutoff百分比
                if(cutoffall.length == 1) {
            ...
            var title = "";
            if(ttest_gene1 != "" & ttest_gene2 != "") {
                title = "          T-Test  of " + glist_ex_[parseInt(ttest_gene1)-1] + " and " + glist_ex_[parseInt(ttest_gene2)-1] + " is " + t_value;
            }
            var option = {
                title: {
