var mutation = new Object(
    {
        URL: {
             ...
             * @return {string}
             */
            TMB: function (datatype, cancer_and_variant, chr, mode) {
                return mutation.URL.BASE_URL() + 'mutation/' + datatype + '/' + cancer_and_variant + '/' + chr + '/' + mode;
            },
            /**
             * TTR接口路径
             * @return {string}
             */
            TTR: function (datatype, strandbias, cancertype, chr, checkedt, mode) {
                return mutation.URL.BASE_URL() + 'mutation/' + datatype + '/' + strandbias + '/' + cancertype + '/' + chr + '/' + checkedt + '/' + mode;
            },
            /**
             * T-Test
             * @return {string}
             */
            TTEST_URL: function () {
                return mutation.URL.BASE_URL() + "test";
            }
        },
        Operate: {
            TMB: function (datatype, cancer_and_variant, chr, mode) {
                if (!datatype && !cancer_and_variant) {
                    $("#bt_draw").disabled = false;
                    $("#echarts").html(
                        '<p class="col-md-12" style="text-align:center;color:rgb(61,132,193);margin-top:25%;">❌ Please select a data type and at least one cancer type & variant type.</p>');
                } else if (!datatype) {
                    $("#bt_draw").disabled = false;
                    $("#echarts").html(
                        '<p class="col-md-12" style="text-align:center;color:rgb(61,132,193);margin-top:25%;">❌ Please select a data type.</p>');
                } else if (!cancer_and_variant) {
                    $("#bt_draw").disabled = false;
                    $("#echarts").html(
                        '<p class="col-md-12" style="text-align:center;color:rgb(61,132,193);margin-top:25%;">❌ Please select at least one cancer type & variant type.</p>');
                } else if (!mutation.Utils.isValidTMB(cancer_and_variant)) {
                    $("#bt_draw").disabled = false;
                    $("#echarts").html(
                        '<p class="col-md-12" style="text-align:center;color:rgb(61,132,193);margin-top:25%;">❌ Please check the cancer type, variant type or use the correct separation symbol.<br />CancerType:VariantType (For example: GBM:SNP). </p>');
                } else {
                    ...
                    $.get(mutation.URL.TMB(datatype, cancer_and_variant, chr, mode), {}, function (data, status) {
                        if (status) {
                            ...
                            //pointSpread填充
                            var pointSpreadData = mutation.Utils.pointSpread(resultData);
                            ...
                            if ($("#del").is(":checked"))
                                vts.push("DEL");
                            var yAxisName = vts.join("+");

                            mutation.View.draw(flag, cancer_types, yAxisName, resultData, beesStyle, is_reRender);
                        } else {
            ...
            TTR: function (datatype, strandbias, cancertype, chr, checkedt, mode) {
                if (!datatype && !cancertype) {
                    $("#bt_draw").disabled = false;
                    $("#echarts").html(
                        '<p class="col-md-12" style="text-align:center;color:rgb(61,132,193);margin-top:25%;">❌ Please select a data type and at least one cancer type.</p>');
                } else if (!datatype) {
                    $("#bt_draw").disabled = false;
                    $("#echarts").html(
                        '<p class="col-md-12" style="text-align:center;color:rgb(61,132,193);margin-top:25%;">❌ Please select a data type.</p>');
                    ...
                    $("#bt_draw").disabled = false;
                    $("#echarts").html(
                        '<p class="col-md-12" style="text-align:center;color:rgb(61,132,193);margin-top:25%;">❌ Please select at least one cancer type.</p>');
                } else if (!mutation.Utils.isValidTTR(checkedt)) {
                    $("#bt_draw").disabled = false;
                    $("#echarts").html(
                        '<p class="col-md-12" style="text-align:center;color:rgb(61,132,193);margin-top:25%;">❌ Please select at least one transition and one transversion.</p>');
                } else {
                    $("#bt_draw").attr("disabled", "true");
                    $("#echarts").html('<div class="spinner">' +
                        '<div class="rect1"></div>' +
                        ...
                        '<div class="rect4"></div>' +
                        '</div><p class="col-md-12" style="text-align:center;color:rgb(61,132,193);">Loading... </br>Due to the big data size and the internet speed, this may take a while.</p>');
                    $.get(mutation.URL.TTR(datatype, strandbias, cancertype, chr, checkedt, mode), {}, function (data, status) {
                        if (status) {
                            var flag = data.data.flag;
                            var resultData = [];
                            //解析
                            ...
                            var beesStyle = [];
                            var is_reRender = false;
                            mutation.View.draw(flag, cancertype, checkedt, resultData, beesStyle, is_reRender);
                        } else {
                            $("#echarts").html('<p class="col-md-12" style="text-align:center;color:rgb(61,132,193);">There is something wrong with server~~~</p>');
                        }
                    });
        ...
        Utils: {
            /**
             * 判断输入的CancerType & VariantType是否正确
             * @param cancer_and_variant 癌症和突变类型
             * @return {boolean}
             */
            isValidTMB: function (cancer_and_variant) {
                var variant_arr = ['SNP', 'DNP', 'TNP', 'ONP', 'INS', 'DEL', 'ALL'];

                var cv_arr = cancer_and_variant.split(',');
                for (var i = 0; i < cv_arr.length; i++) {
                    var cv = cv_arr[i].split(":");
                    if (cv.length !== 2)
                        return false;
                    for (var j in cv[1].split("+")) {
                        if (!variant_arr.includes(cv[1].split("+")[j]))
                            return false;
                    }
                }
                return true;
            },
            isValidTTR: function (trans) {
                var transition_arr = ['ag', 'ga', 'ct', 'tc'];
                var transversion_arr = ['at', 'ta', 'gc', 'cg', 'ac', 'ca', 'gt', 'tg'];

                var t = trans.split(':');
                if (t.length !== 2) {
                    return false;
                } else {
                    if (t[0].split(',').length === 0 || t[1].split(',').length === 0)
                        return false;

                    for (var i in t[0].split(',')) {
                        if (!transition_arr.includes(t[0].split(',')[i]))
                            return false;
                    }
                    for (var j in t[1].split(',')) {
                        if (!transversion_arr.includes(t[1].split(',')[j]))
                            return false;
                    }
                }
                return true;
            ...
            /**
             * 打散函数
             * @param data 后端回传的初始数据
             * @returns {Array} 打散后的数据
             */
            pointSpread: function (data) {
                ...
                for (var kk = 0; kk < data.length; kk++) {
                    ...
                    for (var i = 0; i < result.length; i++) {
                        intermediateData.push(result[i].x);
                    }
                    var finalData = [];
                    for (var i = 0; i < intermediateData.length; i++) {
                        finalData.push(mutation.Utils.normalize(intermediateData[i], Math.max.apply(null, intermediateData), Math.min.apply(null, intermediateData)));
                    }
            ...
            tableData: function (up, q3, avg, std, mid, q1, down, xmin) {
                ...
                viewData = [
                    [{
                        coord: [xmin, mutation.Utils.getResult(up, 4)],
                        symbol: 'none'
                    }, {
                        coord: [xmin, mutation.Utils.getResult(up, 4)],
                        symbol: 'none'
                    ...
                    [{
                        coord: [xmin, mutation.Utils.getResult(q3, 4)],
                        symbol: 'none'
                    }, {
                        coord: [xmin, mutation.Utils.getResult(q3, 4)],
                        symbol: 'none'
                    ...
                    [{
                        coord: [xmin, mutation.Utils.getResult(mid, 4)],
                        symbol: 'none'
                    }, {
                        coord: [xmin, mutation.Utils.getResult(mid, 4)],
                        symbol: 'none'
                    ...
                    [{ //q1
                        coord: [xmin, mutation.Utils.getResult(q1, 4)],
                        symbol: 'none'
                    }, {
                        coord: [xmin, mutation.Utils.getResult(q1, 4)],
                        symbol: 'none'
                    ...
                    [{ //down
                        coord: [xmin, mutation.Utils.getResult(down, 4)],
                        symbol: 'none'
                    }, {
                        coord: [xmin, mutation.Utils.getResult(down, 4)],
                        symbol: 'none'
                    ...
                    [{ //avg
                        coord: [xmin, mutation.Utils.getResult(avg, 4)],
                        symbol: 'none'
                    }, {
                        coord: [xmin, mutation.Utils.getResult(avg, 4)],
                        symbol: 'none'
                    ...
                    [{ //std
                        coord: [xmin, mutation.Utils.getResult(std, 4)],
                        symbol: 'none'
                    }, {
                        coord: [xmin, mutation.Utils.getResult(std, 4)],
                        symbol: 'none'
            ...
            ttest: function (data, gene1, gene2) {
                var testData_ = mutation.Utils.getTestData(data, gene1, gene2);
                //json序列化
                //var jsonObj=json.stringify(testlist);
                var result;
                $.ajax({
                    url: mutation.URL.TTEST_URL(),
                    async: false,
                    type: "post",
                    dataType: "json",
            ...
            getVar: function (valueList) {
                var devAll = 0;
                var mean = mutation.Utils.getMean(valueList);
                for (var i = 0; i < valueList.length; i++) {
        ...
        View: {
            init: function () {
                ...
                $("#cancertype").html("");
                $("#cancertype").append("<option disabled selected value=''>Please select cancer type.</option>");
                /**/
                for (var i = 0; i < cancer_arr.length; i++) {
                    var r = /\[(.+?)]/; //正则获取方括号内容并转为小写
                    $("#cancertype").append("<option class='col-md-12' value='" + cancer_arr[i].match(r)[1].toLowerCase() + "'>" + cancer_arr[i] + "</option>");
                }
                ...
                $("#datatype").html("");
                $("#datatype").append("<option disabled selected value=''>Please select data type.</option>");
                for (var i = 0; i < datatype_arr.length; i++) {
                    $("#datatype").append("<option class='col-md-12' value='" + datatype_arr[i].toLowerCase() + "'>" + datatype_arr[i] + "</option>");
                }
            },
            draw: function (flag, genename, yAxisName, data, beesStyle, is_reRender) {
                $("#echarts").html('<div class="spinner">' +
                    '<div class="rect1"></div>' +
                    '<div class="rect2"></div>' +
                    '<div class="rect3"></div>' +
                ...
                if (ttest_gene1 != "" & ttest_gene2 != "") {
                    t_value = mutation.Utils.ttest(data, ttest_gene1, ttest_gene2);
                }
                ...
                if (!(beesStyle.length)) {
                    ...
                    beesStyle[mutation.Utils.search(glist_ex_, $("#bees_which_select").val())][0] = $("#pointColor_select").val();
                    beesStyle[mutation.Utils.search(glist_ex_, $("#bees_which_select").val())][1] = $("#pointSize_select").val();
                    beesStyle[mutation.Utils.search(glist_ex_, $("#bees_which_select").val())][2] = $("#pointStyle_select").val();
                    beesStyle[mutation.Utils.search(glist_ex_, $("#bees_which_select").val())][3] = $("#boxColor_select").val();
                    beesStyle[mutation.Utils.search(glist_ex_, $("#bees_which_select").val())][4] = $("#boxStyle_select").val();
                    beesStyle[mutation.Utils.search(glist_ex_, $("#bees_which_select").val())][5] = $("#box_lineWidth_select").val() * 1;
                    beesStyle[mutation.Utils.search(glist_ex_, $("#bees_which_select").val())][6] = $("#lineColor_select").val();
                    beesStyle[mutation.Utils.search(glist_ex_, $("#bees_which_select").val())][7] = $("#lineStyle_select").val();
                    beesStyle[mutation.Utils.search(glist_ex_, $("#bees_which_select").val())][8] = $("#mean_lineWidth_select").val() * 1;
                }
                /**样式数组初始化完毕*/
                /**监听重新渲染按钮 重新渲染*/
                $("#btn_Rerender").click(function () {
                    ...
                    //总共几个图
                    var i = mutation.Utils.search(glist_ex_, $("#bees_which_select").val());
                ...
                if (data.length != 0) {
                    for (var dd = 0; dd < data.length; dd++) {
                        ...
                        var avg = mutation.Utils.getMean(value_list);
                        var std = mutation.Utils.getVar(value_list);
                        //cutoff统计百分比
                        ...
                        if ($("#boxplot").is(':checked')) {
                            if ($("#mean").is(':checked') || $("#mean1").is(':checked') || $("#mean2").is(':checked') || $("#mean3").is(':checked')) {
                                linedata = mutation.Utils.box(up, q3, avg, std, mid, q1, down, xmin, xmax, cutoffall, Xmax, beesStyle[dd][3], beesStyle[dd][4], beesStyle[dd][5], beesStyle[dd][6], beesStyle[dd][7], beesStyle[dd][8]);
                                tabledata = mutation.Utils.tableData(up, q3, avg, std, mid, q1, down, xmin);
                            } else {
                                linedata = mutation.Utils.box(up, q3, "", std, mid, q1, down, xmin, xmax, cutoffall, Xmax, beesStyle[dd][3], beesStyle[dd][4], beesStyle[dd][5], beesStyle[dd][6], beesStyle[dd][7], beesStyle[dd][8]);
                                tabledata = mutation.Utils.tableData(up, q3, avg, std, mid, q1, down, xmin);
                            }
                        } else {
                            linedata = mutation.Utils.box("", "", avg, std, "", "", "", xmin, xmax, cutoffall, Xmax, "", "", "", beesStyle[dd][6], beesStyle[dd][7], beesStyle[dd][8]);
                            tabledata = mutation.Utils.tableData(up, q3, avg, std, mid, q1, down, xmin);
                        }
                        //构造cutoff百分比
                        if (cutoffall.length == 1) {
                ...
                var title = "";
                if (ttest_gene1 != "" & ttest_gene2 != "") {
                    title = "          T-Test  of " + glist_ex_[parseInt(ttest_gene1) - 1].split(':')[0] + " and " + glist_ex_[parseInt(ttest_gene2) - 1].split(':')[0] + " is " + t_value;
                }
                var option = {
                    title: {
