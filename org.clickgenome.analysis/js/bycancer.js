  ...
  //请求路径
	URL: {
		...
		BEESWORM: function(genename, cancertype) {
			...
			if($("#simple_nonmalignant").is(':checked') && $("#simple_tumor").is(':checked')) {
				return beesworm.URL.BASE_URL() + genename + "/" + cancertype + "/" + $("#dataType").val() +"/" + $("#newOrOld").val() + "/" + value + "/bycancer";
			} else if($("#simple_tumor").is(':checked')) {
				return beesworm.URL.BASE_URL() + genename + "/" + cancertype + "/" + $("#dataType").val() +"/" + $("#newOrOld").val() + "/" + value + "/bycancer/tumor";
			} else if($("#simple_nonmalignant").is(':checked')) {
				return beesworm.URL.BASE_URL() + genename + "/" + cancertype + "/" + $("#dataType").val() +"/" + $("#newOrOld").val() + "/" + value + "/bycancer/nonmalignant";
			} else {
				return beesworm.URL.BASE_URL() + genename + "/" + cancertype + "/" + $("#dataType").val() +"/" + $("#newOrOld").val() + "/" + value + "/bycancer";
			}
		},
	...
	Operate: {
		BEESWORM: function(genename, cancertype) {
            if(genename == "" && cancertype == ""){
                ...
                $("#bt_draw").disabled=true;
                    $.get(beesworm.URL.CHECK_URL(genename), {}, function(data) {//检查
                        //第一个if:对输入的检查
                        if(data.data) {
                            ...
                            $.get(beesworm.URL.BEESWORM(genename, cancertype), {}, function(data, status) {
                                if(status) {
                                        ...
                                        beesworm.View.draw(flag, genename, cancertype, resultData, beesStyle, is_reRender);
                                } else {
	...
	View: {
		...
		draw: function(flag ,genename, cancertype, data, beesStyle, is_reRender) {
			...
			/**初始化图片下载配置结束*/
			//样式数组初始化
            //console.log('glist_ex_哈哈'+glist_ex_);
			if(beesStyle.length == 0) {
                ...
                beesStyle[beesworm.Utils.search(glist_ex_, $("#bees_which_select").val())][5] = $("#box_lineWidth_select").val();
            ...
            // console.log("series的长度 "+series.length);
